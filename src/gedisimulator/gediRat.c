...
int main(int argc,char **argv)
{
  ...
  for(i=0;i<dimage->gediIO.nFiles;i++){
    ...
    if(dimage->gediRat.doGrid||dimage->gediRat.readALSonce)Rprintf("File %d of %d",i+1,dimage->gediIO.nFiles);
    /*read lasFile*/
    ...
    if(dimage->gediRat.doGrid||dimage->gediRat.readALSonce)Rprintf(" nPoints %u\n",data[i]->nPoints);
