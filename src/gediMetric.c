...
control* createControl(const char *input, const char *output) {
  control *dimage=NULL;
...
SEXP processFloWave2(SEXP input, SEXP output) {
  const char* input_str = CHAR(asChar(input));
  const char* output_str = CHAR(asChar(output));
  int i=0;
  control *dimage=NULL;
  ...
  dimage = createControl(input_str, output_str);
  ...
  for(i=0;i<dimage->gediIO.nFiles;i++){
    if((i%dimage->gediIO.nMessages)==0) Rprintf(\r"Wave %d of %d          ",i+1,dimage->gediIO.nFiles);
    /*read waveform*/
...
void writeResults(dataStruct *data,control *dimage,metStruct *metric,int numb,float *denoised,float *processed,char *inNamen)
{
  ...
  if(dimage->gediIO.gFit->nGauss>dimage->maxGauss)Rprintf("More Gaussians than header entries %d\n",dimage->gediIO.gFit->nGauss);
